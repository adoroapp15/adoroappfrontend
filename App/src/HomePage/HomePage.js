...
//       return <Relevant navigation={navigation} ref={flatListRef} data={posts}/>;
//     } else {
...
const HomePage = ({navigation, route}) => {
    ...
    try {
      ...
      if (userString && otherString) {
        ...
        if (response.data.status === 200) {
          ...
          const res = await axios.get(
            `${config.production}/app/user/userinterest`,
            {params: {interest: interest, UserId: parsedUser.Id, page: pageNumber}},
          );
          if (res.data.status === 200) {
            const userInterests = response.data.data.Interest.split(' ');

            setPosts(prevPosts => pageNumber === 1 ? res.data.posts : [...prevPosts, ...res.data.posts]);
  ...
  const renderSelectedContent = () => {
    if (selectedButton === 'Trending') {
      ...
      return <Relevant navigation={navigation} ref={flatListRef} data={posts} onEndReached={onEndReached}/>;
    } else {
      const filteredPosts = posts.filter(
        item => item.category === selectedButton,
      );
